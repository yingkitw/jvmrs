...
use crate::error::{JvmError, ClassLoadingError, to_class_loading_error};
...
impl ClassLoader {
    ...
    pub fn load_class(&mut self, class_name: &str) -> Result<(), JvmError> {
        ...
        for classpath_dir in &self.classpath {
            ...
            if full_path.exists() {
                match ClassFile::from_file(&full_path) {
                    Ok(class_file) => {
                        let actual_class_name = class_file.get_class_name()
                            .unwrap_or_else(|| class_name.to_string());
                        
                        // Verify class name matches
                        if actual_class_name != class_name {
                            return Err(to_class_loading_error(
                                ClassLoadingError::ClassFormatError(
                                    format!("Class name mismatch: expected {}, got {}", 
                    ...
                    Err(e) => {
                        return Err(to_class_loading_error(
                            ClassLoadingError::ClassFormatError(
                                format!("Failed to parse class file {}: {:?}", 
                                        full_path.display(), e)
                            )
                        ));
                    }
        ...
        // Class not found
        Err(to_class_loading_error(
            ClassLoadingError::NoClassDefFound(class_name.to_string())
        ))
    }
